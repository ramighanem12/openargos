  ...
  return {
    ...
    async execute(action, capture) {
      ...
      if (type === "click" || type === "double_click") {
        ...
        await sleep(randomIntBetween(computerUseActionMicroDelayMinMs, computerUseActionMicroDelayMaxMs));
        await client.call("Input.dispatchMouseEvent", { type: "mousePressed", x: point.x, y: point.y, button: normalizeComputerActionButton(action.button), clickCount });
        await sleep(randomIntBetween(computerUseActionMicroDelayMinMs, computerUseActionMicroDelayMaxMs));
        await client.call("Input.dispatchMouseEvent", { type: "mouseReleased", x: point.x, y: point.y, button: normalizeComputerActionButton(action.button), clickCount });
        return;
      }
      if (type === "scroll") {
        const point = mapBackgroundBrowserPoint(action, capture);
      ...
      if (type === "type") {
        const text = String(action.text || "");
        if (text) await client.call("Input.insertText", { text });
        return;
      }
